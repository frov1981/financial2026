...
const getCategoryBase = async (user_id: number): Promise<CategoryGroup[]> => {
...
export const deleteCategoryGroupCache = (auth_req: AuthRequest): void => {
    const cache_key = cacheKeys.categoryGroupByUser(auth_req.user.id)
    const cache_key_api = cacheKeys.categoryGroupByUserForApi(auth_req.user.id)
    const cache_key_category = cacheKeys.categoriesByUser(auth_req.user.id)
    const cache_key_category_api = cacheKeys.categoriesByUserForApi(auth_req.user.id)
    cache.del(cache_key_api)
    cache.del(cache_key)
    cache.del(cache_key_category_api)
    cache.del(cache_key_category)
}

export const getCategoryGroup = async (auth_req: AuthRequest): Promise<CategoryGroup[]> => {
    const user_id = auth_req.user.id
    const category_group: CategoryGroup[] = await getCategoryBase(user_id)
    return category_group
}
