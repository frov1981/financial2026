...
function toggleCard(id) {
  ...
  icon.innerHTML = isNowHidden ? iconChevronClose() : iconChevronOpen()

  const state = loadFilters(CARD_STATE_KEY) || {}
  state[id] = !isNowHidden
  saveFilters(CARD_STATE_KEY, state)

  // Resize charts cuando se hacen visibles
  if (id === 'chart-six' && !isNowHidden && lastSixMonthsChart) lastSixMonthsChart.resize()
  if (id === 'chart-years' && !isNowHidden && lastSixYearsChart) lastSixYearsChart.resize()
  if (id === 'chart-loans' && !isNowHidden && lastLoansChart) lastLoansChart.resize()
}
