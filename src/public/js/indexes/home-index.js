...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    Object.entries(savedState).forEach(([id, isOpen]) => {
        ...
        if (!isOpen) {
            ...
            icon.innerHTML = iconChevronClose()
        }
    })
    ...
    try {
        ...
        const { kpis, lastSixMonthsChartData, lastSixYearsChartData, globalKpis } = await res.json()
        // ============================
        // KPIs Globales
        // ============================
        ...
        if (typeof Chart !== 'undefined') {
            ...
            lastSixMonthsChart = new Chart(ctx, {
                type: 'line',
                data: {
            ...
            if (typeof Chart !== 'undefined') {
                const ctxYears = document
                    .getElementById('lastSixYearsChart')
                    .getContext('2d')
                ...
                lastSixYearsChart = new Chart(ctxYears, {
                    ...
                    data: {
                        ...
                        datasets: [
                            {
                                label: 'Ingresos',
                                data: lastSixYearsChartData.income,
                                tension: 0.35
                            },
                            {
                                label: 'Egresos',
                                data: lastSixYearsChartData.expense,
                                tension: 0.35
                            },
                            {
                                label: 'Balance',
                                data: lastSixYearsChartData.balance,
                                borderDash: [6, 4],
                                tension: 0.35
                            }
                        ]
...
function toggleCard(id) {
    ...
    icon.innerHTML  = isHidden ? iconChevronClose() : iconChevronOpen()

    const state = loadFilters(CARD_STATE_KEY) || {}
    state[id] = isHidden
    saveFilters(CARD_STATE_KEY, state)

    if (id === 'chart-six' && isHidden && lastSixMonthsChart) {
        lastSixMonthsChart.resize()
    }

    if (id === 'chart-years' && isHidden && lastSixYearsChart) {
        lastSixYearsChart.resize()
    }
}
