...
function randomDecemberDate(): Date {
...
async function runSeed(ds: DataSource): Promise<void> {
    ...
    for (let u = 1; u <= 3; u++) {
        ...
        const categories = await categoryRepo.save([
            { user, name: 'Salario', type: 'income' as const, parent: null },
            { user, name: 'Alimentación', type: 'expense' as const, parent: null },
            { user, name: 'Transporte', type: 'expense' as const, parent: null },
            { user, name: 'Pago préstamo', type: 'expense' as const, parent: null }
        ].slice(0, random(3, 5)))
