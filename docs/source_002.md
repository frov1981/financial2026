...
import { injectPayableBalance } from './middlewares/inject-loan-balance.middleware'
...
protectedRouter.use('/loans', injectPayableBalance, loanRoutes)
...
import { AppDataSource } from '../../config/typeorm.datasource'
import { PayableGroup } from '../../entities/PayableGroup.entity'
import { loanGroupFormMatrix } from '../../policies/payable-group-form.policy'
import { AuthRequest } from '../../types/auth-request'
...
import { PayablePayment } from '../../entities/PayablePayment.entity';
import { Transaction } from '../../entities/Transaction.entity';
import { paymentFormMatrix } from '../../policies/payable-payment-form.policy';
import { KpiCacheService } from '../../services/kpi-cache.service';
...
export const getActiveParentPaymentsByUser = async (auth_req: AuthRequest): Promise<PayableGroup[]> => {
  const repo = AppDataSource.getRepository(PayableGroup)
...
export const getActiveCategoriesPaymentsByUser = async (authReq: AuthRequest): Promise<Category[]> => {
  const repo = AppDataSource.getRepository(Category)
  const categories = await repo.find({
    where: {
      user: { id: authReq.user.id },
      is_active: true,
      type_for_payable: 'payment'
    },
    order: { name: 'ASC' }
  })
  return categories
