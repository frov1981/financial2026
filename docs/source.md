...
export const getActivePayableCategories = async (auth_req: AuthRequest): Promise<Category[]> => {
    const user_id = auth_req.user.id
    const categories: Category[] = await getCategoriesBase(user_id)
    const active_payable_categories: Category[] = categories.filter(category => category.is_active && category.type_for_payable_or_receivable === 'payable')
...
export const getActivePaymentCategories = async (auth_req: AuthRequest): Promise<Category[]> => {
    const user_id = auth_req.user.id
    const categories: Category[] = await getCategoriesBase(user_id)
    const active_payment_categories: Category[] = categories.filter(category => category.is_active && category.type_for_payable_or_receivable === 'payable_payment')
...
export const getCategoriesForApi = async (auth_req: AuthRequest): Promise<DTOCategory[]> => {
    ...
    const categories: DTOCategory[] = result.entities.map((category, index) => ({
        ...
        name: category.name,
        type: category.type,
        type_for_payable_or_receivable: category.type_for_payable_or_receivable,
...
export const cacheKeys = {
  ...
  /*Payable Payments*/
  payablePaymentsByUser: (user_id: number) => `payable_payment_user_${user_id}`,
  payablePaymentsByUserForApi: (user_id: number) => `payable_payment_api_user_${user_id}`,
  payablePaymentsByPayment: (user_id: number, payment_id: number) => `payable_payment_user_${user_id}_payment_${payment_id}`,
  payablePaymentsByPayableForApi: (user_id: number, payable_id: number) => `payable_payment_api_user_${user_id}_payable_${payable_id}`,
  payablePaymentsByPayablePrefix: (user_id: number) => `payable_payment_api_user_${user_id}_payable_`,
  /*Receivables*/
  ...
  allByUser: (user_id: number) => [
    ...
    `payable_payments_user_${user_id}`,
    `payable_payments_api_user_${user_id}`,
    /*Receivables*/
    `receivables_user_${user_id}`,
    `receivables_api_user_${user_id}`,
    `receivable_categories_user_${user_id}`,
    `receivable_collection_categories_user_${user_id}`,
    `receivable_group_user_${user_id}`,
...
export const getActiveCategoriesForPayablePaymentsByUser = async (auth_req: AuthRequest): Promise<Category[]> => {
    const user_id = auth_req.user.id
    const cache_key = cacheKeys.payablePaymentCategoriesByUser(user_id)
    const cached_categories = cache.get<Category[]>(cache_key)
    if (cached_categories !== undefined) {
        return cached_categories
    }
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\cache\cache-receivable-collections.service.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\cache\cache-receivable-groups.service.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\cache\cache-receivables.service.ts
```
...
import { getCategoryById } from '../../cache/cache-categories.service';
import { deleteAll } from '../../cache/cache-key.service';
import { getActiveCategoriesForPayablePaymentsByUser, getPaymentById } from '../../cache/cache-payable-payments.service';
import { getPayableById } from '../../cache/cache-payables.service';
import { AppDataSource } from '../../config/typeorm.datasource';
import { Account } from '../../entities/Account.entity';
import { Payable } from '../../entities/Payable.entity';
...
export const validateSavePayment = async (auth_req: AuthRequest, payment: PayablePayment, old_payment: PayablePayment | null): Promise<Record<string, string> | null> => {
    const payment_instance = plainToInstance(PayablePayment, payment)
    const errors = await validate(payment_instance)
    const field_errors = errors.length > 0 ? mapValidationErrors(errors) : {}

    const payment_repo = AppDataSource.getRepository(PayablePayment)
    // Validación monto principal
    let available_amount = payment.payable.balance
    if (old_payment) available_amount += old_payment.principal_paid
    if (payment.principal_paid > available_amount) {
        field_errors.principal_paid = 'El monto del capital supera el saldo pendiente de la cuenta por pagar'
    }
    ...
    const last_payment = await payment_repo.findOne({ where: { payable: { id: payment.payable.id } }, order: { payment_date: 'DESC', id: 'DESC' } })
...
export const validateDeletePayment = async (auth_req: AuthRequest, payment: PayablePayment): Promise<Record<string, string> | null> => {
    const field_errors: Record<string, string> = {}
    const now = new Date()
    ...
    const last_payment = await payment_repo.findOne({
        where: { payable: { id: payment.payable.id } },
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable\receivable.controller.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable\receivable.saving.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable\receivable.validator.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable-collection\receivable-collection.controller.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable-collection\receivable-collection.saving.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable-collection\receivable-collection.validator.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable-group\receivable-group.controller.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable-group\receivable-group.saving.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\controllers\receivable-group\receivable-group.validator.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\middlewares\inject-payable-balance.middleware.ts
```
...
export const categoryFormMatrix: CategoryFormMatrix = {
    insert: {
        type: 'editable',
        type_for_payable_or_receivable: 'editable',
        name: 'editable',
    ...
    delete: {
        type: 'readonly',
        type_for_payable_or_receivable: 'readonly',
        name: 'readonly',
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\policies\payable-form.policy.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\policies\payable-group-form.policy.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\policies\payable-payment-form.policy.ts
```
 
```ts
import { PayablePaymentFormMatrix } from "../types/form-view-params";

export const payablePaymentFormMatrix: PayablePaymentFormMatrix = {
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\policies\payable-receivable_collection-form.policy.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\policies\receivable-form.policy.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\policies\receivable-group-form.policy copy.ts
```
...
export interface RoleUser {
    can_update_amount_payable?: boolean
    can_update_start_date_payable?: boolean
...
export const role_permissions: Record<Role, RoleUser> = {
    ADMIN: {
        can_update_amount_payable: true,
        can_update_start_date_payable: true,
        can_update_date_payment: true,
        can_update_date_transaction: true,
        can_delete_transaction: true,
    },
    USER: {
        can_update_amount_payable: false,
        can_update_start_date_payable: false,
...
@import url('./modules/payables.css');
@import url('./modules/payable-payments.css');
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\public\css\modules\payable-payments.css
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\public\css\modules\payables.css
```
...
.payable-card.card-selected::after {
    content: '';
    position: absolute;
    inset: 0;
    border: 2px solid var(--ui-blue-600);
...
.payable-card.card-selected {
    border-color: transparent;
}

.payable-card.inactive {
    background: var(--ui-red-50);
}

.payable-card .icon-btn {
    padding: 0.375rem;
    min-height: auto;
}

.payable-card .card-header {
    display: flex;
    justify-content: space-between;
    align-items: center;
}

.payable-card .card-title {
    font-size: 15px;
    font-weight: 600;
}

.payable-card .card-balance {
    margin: 8px 0;
    font-size: 22px;
    font-weight: 600;
}

.payable-card .card-sub {
    font-size: 12px;
    color: var(--ui-gray-500);
}

.payable-card .card-actions {
    display: flex;
    gap: 4px;
}

.payable-card .card-footer {
    display: flex;
    justify-content: space-between;
    align-items: center;
    margin-top: 6px;
...
.payable-card .tag-line {
    text-align: right;
}

.payable-card .tag-line:not(:first-child) {
    font-size: 12px;
    color: var(--ui-gray-500);
}
...
.payable-group-header .group-actions {
    display: flex;
    gap: 4px;
    margin-left: auto;
}
...
.payable-group.collapsed .payable-group-body {
    display: none;
}
...
.payable-card .date-block {
    display: flex;
    flex-direction: column;
    line-height: 1.2;
}

.payable-card .weekday-text {
    color: var(--ui-gray-500);
}

.payable-card .payable-amounts {
    display: flex;
    justify-content: space-between;
    gap: 8px;
}

.payable-card .payable-amount-item {
    flex: 1;
}

.payables-group-header-left {
    display: flex;
    align-items: center;
}

.payables-group-center {
    display: flex;
    flex-direction: column;
    justify-content: center;
    margin-left: 8px;
...
.payables-group-title {
    font-weight: 600;
    line-height: 1.2;
}

.payables-group-pending {
    font-size: 12px;
    font-weight: 500;
    opacity: 0.85;
    line-height: 1.1;
...
.payable-card .payable-amount-title {
    text-align: left;
    font-size: 11px;
    color: var(--ui-gray-500);
}

.payable-card .payable-amount-value {
    font-weight: 600;
    font-size: 13px;
} 
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\public\js\indexes\payable-payments-index.js
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\public\js\indexes\payables-index.js
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\routes\payable-group.route.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\routes\payable-payment.route.ts
```
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\routes\payable.route.ts
```
...
<div class="max-w-xl mx-auto">
  ...
  <form method="post" action="/categories/">
    ...
    <div class="mb-4">
      ...
      <div class="flex gap-6">
        ...
        <label class="flex items-center gap-1">
          <input
            ...
            name="type_for_payable_or_receivable"
            value=""
            <%= category?.type_for_payable_or_receivable == null ? 'checked' : '' %>
            <%= isReadOnly('type_for_payable_or_receivable') ? 'disabled' : '' %>
          >
          Sin tipo
        </label>
      ...
      <% if (isReadOnly('type_for_payable_or_receivable')) { %>
        <input type="hidden" name="type_for_payable_or_receivable" value="<%= category?.type_for_payable_or_receivable || '' %>">
      <% } %>

      <% if (errors?.type_for_payable_or_receivable) { %>
        <p class="text-red-600 text-sm mt-1"><%= errors.type_for_payable_or_receivable %></p>
      <% } %>
    </div>
    <% } %>
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\views\pages\payable-groups\form.ejs
```
 
```ejs
<%
  const rules = payable_group_form_policy
  const isHidden = field => rules[field] === 'hidden'
...
<div class="max-w-xl mx-auto">
  <h1 class="text-2xl font-semibold mb-6">
    <%= 
      mode === 'insert' ? 'Insertar Grupo Cuentas por Pagar' :
      mode === 'update' ? 'Editar Grupo Cuentas por Pagar' :
      mode === 'delete' ? 'Eliminar Grupo Cuentas por Pagar' :
      mode
    %>
  </h1>

  <form method="post" action="/payable-groups/">
    <!-- Token CSRF -->
    <input type="hidden" name="_csrf" value="<%= csrfToken %>">
    ...
    <!-- ID y acción -->
    <input type="hidden" name="id" value="<%= payable_group?.id || '' %>">
    <input type="hidden" name="mode" value="<%= mode %>">
    ...
    <% if (!isHidden('is_active')) { %>
      <div class="mb-6">
        ...
        <select 
          ...
          <%= isReadOnly('is_active') ? 'disabled' : '' %>
        >
          <option value="true" <%= payable_group?.is_active ? 'selected' : '' %>>Activar</option>
          <option value="false" <%= !payable_group?.is_active ? 'selected' : '' %>>Inactivar</option>
        </select>
        <% if (errors?.is_active) { %>
          <p class="text-red-600 text-sm mt-1"><%= errors.is_active %></p>
        <% } %>
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\views\pages\payable-payments\form.ejs
```
...
<script src="/js/forms/autocomplete-form.js"></script>
<script src="/js/forms/payable-payment-form.js"></script> 
```
 
--- 
 
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\views\pages\payable-payments\index.ejs
```
 
```ejs
<script>
  window.USER_ID = "<%= USER_ID %>"
  window.PAYABLE_ID = "<%= payable.id %>"
</script>
  ...
  <div class="ui-header">
    ...
    <div class="ui-toolbar">
      ...
      <%- include('../../partials/btn-back', {
        href: '/payables',
        title: 'Volver a Cuentas por Pagar',
        text: 'Volver'
      }) %>

      <%- include('../../partials/btn-new', {
        href: `/payments/insert/${payable.id}`,
        title: 'Nuevo pago',
        text: 'Nuevo',
        data_btn: 'new'
      }) %>
...
<script src="/js/indexes/payable-payments-index.js"></script>
<script src="/js/helpers/icon-helper.js"></script>
<script src="/js/helpers/amount-helper.js"></script>
<script src="/js/helpers/type-tags-helper.js"></script>
...
```ejs
<%
  const rules = payable_form_policy
  const isHidden = field => rules[field] === 'hidden'
  const isReadOnly = field => rules[field] === 'readonly'
  const isEditable = field => rules[field] === 'editable'
%>
...
<div class="max-w-xl mx-auto">
  <h1 class="text-2xl font-semibold mb-6">
    <%= 
      mode === 'insert' ? 'Insertar Cuentas por Pagar' : 
      mode === 'update' ? 'Editar Cuentas por Pagar' :
      mode === 'delete' ? 'Eliminar Cuentas por Pagar' : 
      ''
    %>
  </h1>
  ...
  <form method="post" action="/payables/">
    ...
    <input type="hidden" name="mode" value="<%= mode %>">
    <input type="hidden" name="is_active" value="<%= payable?.is_active || 'true' %>">
    <input type="hidden" name="return_from" value="<%= context?.from || '' %>">
    <input type="hidden" name="return_category_id" value="<%= context?.category_id || '' %>">
    ...
    <div class="mb-4">
      ...
      <div
        ...
        data-default-id="<%= payable?.disbursement_account?.id || '' %>"
        data-placeholder="-- Escoja una opción --"
      >
        <input 
          type="text" 
          class="autocomplete-input" 
          autocomplete="off" 
          value="<%= payable?.disbursement_account?.name || '' %>" 
        ...
        <input 
          ...
          value="<%= payable?.disbursement_account?.id || '' %>">
    ...
    <div class="mb-4">
      ...
      <div
        ...
        data-default-id="<%= payable?.category?.id || '' %>"
        data-placeholder="-- Escoja una opción --"
      >
        <input 
          type="text" 
    ...
    <!-- Grupo de Cuentas por Pagar -->
     <% if (!isHidden('payable_group_id')) { %>
    <div class="mb-4">
      <label class="block font-medium mb-1">Grupo de Cuentas por Pagar</label>
      <div
        class="autocomplete"
        data-items='<%- JSON.stringify(payable_group_list) %>'
        data-default-id="<%= payable?.payable_group || '' %>"
        data-placeholder="-- Escoja una opción --"
      >
        <input 
          type="text" 
          class="autocomplete-input" 
          autocomplete="off" 
          value="<%= payable?.payable_group?.name || '' %>" 
          <%= isReadOnly('payable_group_id') ? 'readonly' : '' %>>
        
        <input 
          type="hidden" 
          class="autocomplete-hidden" 
          name="payable_group_id"
          value="<%= payable?.payable_group?.id || '' %>">
          
        <% if (!isReadOnly('payable_group_id')) { %>
        <div class="autocomplete-panel"></div>
        <% } %>
      </div>
      <% if (errors?.payable_group) { %>
        <p class="text-red-600 text-sm mt-1"><%= errors.payable_group %></p>
      <% } %>
    </div>
    <% } %>    
...
<script src="/js/forms/payable-form.js"></script>
<script src="/js/forms/autocomplete-form.js"></script>
...
```text
FILE: C:\Users\Dell\Documents\Proyectos\ssrfinan\src\views\pages\payables\index.ejs
```
...
<div class="max-w-6xl mx-auto ui-page">
  ...
  <!-- Header -->
  <div class="ui-header">
    <h1 class="ui-title">Cuentas por Pagar</h1>

    <div class="ui-toolbar">
      <%- include('../../partials/btn-new', {
        href: '#',
        title: 'Nuevo Cuentas por Pagar',
        text: 'Nuevo',
        data_btn: 'new'
      }) %>
    ...
    <%- include('../../partials/ui-modal',{
      ...
      title:'¿Qué desea insertar?',
      buttons:[
        {id:'insert-group', text:'Grupo de Cuentas por Pagar', variant:'ui-modal-btn-primary'},
        {id:'insert-child', text:'Cuentas por Pagar Hijo', variant:'ui-modal-btn-success'},
        {id:'close-modal', text:'Cancelar', variant:'ui-modal-btn-neutral'}
      ]
    }) %>    
  </div>
  ...
  <div class="ui-scroll-area">
    ...
    <!-- Mobile cards -->
    <div id="payables-mobile" class="payables-mobile"></div>
...
<script src="/js/indexes/payables-index.js"></script>
<script src="/js/helpers/icon-helper.js"></script>
<script src="/js/helpers/amount-helper.js"></script>
<script src="/js/helpers/type-tags-helper.js"></script>
<script src="/js/helpers/status-toggle-helper.js"></script>
...
<nav class="bg-blue-600 text-white">
  <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 h-14 flex items-center justify-between">
    ...
    <% if (typeof payable_balance !== 'undefined') { %>
      <span class="navbar-payable_balance">
      <%= payable_balance.toFixed(2) %>
      </span>
    <% } %>
