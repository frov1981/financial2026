...
exports.recalculateAllAccountBalances = void 0;
...
*/
const recalculateAllAccountBalances = async (authReq) => {
    const userId = authReq.user.id;
    const queryRunner = datasource_1.AppDataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
        const accounts = await queryRunner.manager.find(Account_entity_1.Account, {
        ...
        const transactions = await queryRunner.manager
            .createQueryBuilder(Transaction_entity_1.Transaction, 'tx')
        ...
        for (const tx of transactions) {
            ...
            if (tx.type === 'transfer') {
                ...
                if (fromAcc)
                    fromAcc.balance -= Number(tx.amount);
                if (toAcc)
                    toAcc.balance += Number(tx.amount);
            }
        }
        for (const account of accounts) {
            await queryRunner.manager.update(Account_entity_1.Account, { id: account.id }, { balance: account.balance });
        }
        await queryRunner.commitTransaction();
    }
    catch (error) {
